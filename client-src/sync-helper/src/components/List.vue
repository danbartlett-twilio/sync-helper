<script>
  ...
  export default {
    ...
    setup() {
      ...
      // GET LIST DETAILS FROM SyncHelperMain 
      const getListData = async () => {
        
        try {
          if (syncStore?.syncHelperSID !== null) {
            let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-get-array?targetList=${syncStore.syncHelperSID}&filterField=listSid&filterFieldValue=${tableSid}`;
      ...
      const getListItems = async () => {
        ...
        try {          
          let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-get-array?targetList=${tableSid}&ts=${Date.now()}`;
          const res = await fetch(url, { method: "GET", cache: "no-store", headers: {'Content-type': 'application/json'} });
          if (res.ok) {
            let r = await res.json();
      ...
      const saveNewItemObject = async (p) => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-create?targetList=${tableSid}`;
      ...
      const deleteItem = async (i) => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-delete?targetList=${tableSid}&listItemIndex=${i}`;
