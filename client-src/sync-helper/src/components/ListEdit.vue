  ...
  export default {
    setup() {
      ...
      const createList = async() => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-create?uniqueName=${pageObject.listName}`;
        ...
        if (res.ok) {
          ...
          await Promise.all(pageObject.items.map(async (item) => {
            ...
            toast.success("Success! New rows added! Updating Sync Helper Main Table...");
          })          


          // #4 Add a item into the SyncHelperMain for new List
          url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-create?targetList=${syncStore.syncHelperSID}`;          
          let payload = {
      ...
      const saveNewItemObject = async (p) => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-create?targetList=${pageObject.listSid}`;
      ...
      const deleteListFn = async() => {
        ...
        // #1 Delete the List
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-delete?listSid=${pageObject.listName}`;
        const res = await fetch(url, { method: "POST", headers: {'Content-type': 'application/json'} });
            
        if (res.ok) {

          // #2 Delete the meta item from SyncHelperMain      
              
          let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-delete?targetList=${syncStore.syncHelperSID}&listItemIndex=${pageObject.listItemIndex}`;
          const res = await fetch(url, { method: "POST", headers: {'Content-type': 'application/json'} });
          if (res.ok) {
      ...
      const updateList = async() => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-update?targetList=${syncStore.syncHelperSID}`;
      ...
      const getListData = async() => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-item-get-array?targetList=${syncStore.syncHelperSID}&ts=${Date.now()}`;
