  ...
  export default {
    setup() {
      ...
      const getListList = async () => {
        ...
        try {      
          let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-all?ts=${Date.now()}`;
          ...
          if (res.ok) {
            ...
            listList.lists = r;
            syncStore.syncHelperSID = listList.lists.find(l => l.uniqueName === 'SyncHelperMain').sid;
          }          
      ...
      const createMainList = async () => {        
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/list-create?uniqueName=SyncHelperMain`;
        const res = await fetch(url, { method: "POST", headers: {'Content-type': 'application/json'} });            
        if (res.ok) {  
          toast.success("Success! SyncHelperMain created. Waiting 5 seconds before updating...");      
    ...
    <div class="card">
      ...
      <div class="card-body" v-show="isMainList">
        ...
        <table class="table table-striped align-middle">
          ...
          <tbody>
            <tr v-for="l in listList.lists" v-bind:key="l.sid" v-show="l.uniqueName !== 'SyncHelperMain'">
      ...
      <div class="card-body" v-show="!isMainList">
        <div class="alert alert-danger mb-4">
          <p class="fw-bolder">You need to add "SyncHelperMain" to your sync service!</p>
          <p>
            Sync Helper uses a list to store meta data about all of the tables (Sync Lists)
        ...
        <div class="d-grid">
          <button @click="createMainList()" class="btn btn-success btn-lg">Create "SyncHelperMain"</button>
        </div>                
