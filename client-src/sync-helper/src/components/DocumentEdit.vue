  ...
  export default {
    setup() {
      ...
      const getDocument = async () => {
        ...
        try {
          let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/document-get?documentSid=${documentSid}`;
      ...
      const updateDocument = async() => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/document-update?documentSid=${documentSid}`;
      ...
      const createDocument = async() => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync-helper/document-create?documentName=${newDocName.value}`;
        let payload = prettyJson.value;
        const res = await fetch(url, { method: "POST", headers: {'Content-type': 'application/json'}, body: JSON.stringify({"payload": payload}) });
        if (res.ok) {
          let r = await res.json();
